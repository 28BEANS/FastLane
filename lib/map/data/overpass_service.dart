...
class OverpassService {
  ...
  static final Map<String, List<OfficeLocation>> _cache = {};
  ...
  Future<List<OfficeLocation>> getNearbyOffices({
    ...
    // Round coordinates slightly to 4 decimal places (~11 meters) to optimize cache hits
    final roundedLat = double.parse(userLat.toStringAsFixed(4));
    final roundedLon = double.parse(userLon.toStringAsFixed(4));
    ...
    try {
      ...
      final officeLocations = elements.map((e) {
        double lat = 0.0;
        double lon = 0.0;
        if (e['type'] == 'node') {
